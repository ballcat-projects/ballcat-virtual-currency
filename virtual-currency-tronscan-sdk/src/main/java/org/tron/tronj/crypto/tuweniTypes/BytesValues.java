...
final class BytesValues {
	...
	static byte[] fromRawHexString(CharSequence str, int destSize, boolean lenient) {
		...
		if (destSize < 0) {
			...
			checkArgument(size <= destSize, "Hex value is too large: expected at most %s bytes but got %s", destSize,
					size);
		}
		...
		for (int i = 0; i < len; i += 2) {
			...
			if (h == -1) {
				throw new IllegalArgumentException(
						String.format("Illegal character '%c' found at index %d in hex binary representation",
								hex.charAt(i), i - idxShift));
			}
			if (l == -1) {
				throw new IllegalArgumentException(
						String.format("Illegal character '%c' found at index %d in hex binary representation",
								hex.charAt(i + 1), i + 1 - idxShift));
			}
