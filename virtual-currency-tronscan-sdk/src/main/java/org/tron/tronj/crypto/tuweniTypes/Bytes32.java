...
public interface Bytes32 extends Bytes {
	 ...
	 * {@code value} will be reflected in the returned value.
	 ...
	 * <p>
	 * Note that value is not copied, only wrapped, and thus any future update to
	 * {@code value} within the wrapped parts will be reflected in the returned value.
	 * @param bytes The bytes to wrap.
	 ...
	 * {@code wrap(value, i).get(0) == value[i]}.
	 * @return A {@link Bytes32} that exposes the bytes of {@code value} from
	 * {@code offset} (inclusive) to {@code offset + 32} (exclusive).
	 * @throws IndexOutOfBoundsException if
	 * {@code offset < 0 || (value.length > 0 && offset >=
	 *     value.length)}.
	 * @throws IllegalArgumentException if
	 * {@code length < 0 || offset + 32 > value.length}.
	 */
	 ...
	 * @param offset The index (inclusive) in {@code value} of the first byte exposed by
	 * the returned value. In other words, you will have
	 * {@code wrap(value, i).get(0) == value.get(i)}.
	 * @return A {@link Bytes32} that exposes the bytes of {@code value} from
	 * {@code offset} (inclusive) to {@code offset + 32} (exclusive).
	 * @throws IndexOutOfBoundsException if
	 * {@code offset < 0 || (value.size() > 0 && offset >=
	 *     value.size())}.
	 * @throws IllegalArgumentException if
	 * {@code length < 0 || offset + 32 > value.size()}.
	 */
	 ...
	 * left padded with zeros (see {@link #fromHexStringStrict} if this is not what you
	 * want).
	 ...
	 * @return The value corresponding to {@code str}.
	 * @throws IllegalArgumentException if {@code str} does not correspond to a valid
	 * hexadecimal representation, is of an odd length, or contains more than 32 bytes.
	 */
	 ...
	 *
	 * <p>
	 * This method is extra strict in that {@code str} must of an even length and the
