 ...
 *
 * <p>
 * This interface makes no thread-safety guarantee, and a {@link Bytes} value is generally
 * not thread safe. However, specific implementations may be thread-safe. For instance,
 * the value returned by {@link #copy} is guaranteed to be thread-safe as it is immutable.
 */
public interface Bytes extends Comparable<Bytes> {
	 ...
	 *
	 * <p>
	 * Note that value is not copied and thus any future update to {@code value} will be
	 ...
	 * @return A value containing the result of concatenating the value from
	 ...
	 *
	 * <p>
	 * Note that any change to the content of the byteBuf may be reflected in the returned
	 * value.
	 ...
	 * @throws IllegalArgumentException if {@code value < 0} or {@code value} is too big
	 * to fit an unsigned 2-byte short (that is, if {@code value >= (1 << 16)}).
	 */
	...
	static Bytes ofUnsignedShort(int value, ByteOrder order) {
		checkArgument(value >= 0 && value <= BytesValues.MAX_UNSIGNED_SHORT,
				"Value %s cannot be represented as an unsigned short (it is negative or too big)", value);
		byte[] res = new byte[2];
		if (order == BIG_ENDIAN) {
	 ...
	 * @throws IllegalArgumentException if {@code value < 0} or {@code value} is too big
	 * to fit an unsigned 4-byte int (that is, if {@code value >= (1L << 32)}).
	 */
	...
	static Bytes ofUnsignedInt(long value, ByteOrder order) {
		checkArgument(value >= 0 && value <= BytesValues.MAX_UNSIGNED_INT,
				"Value %s cannot be represented as an unsigned int (it is negative or too big)", value);
		byte[] res = new byte[4];
		if (order == BIG_ENDIAN) {
	 ...
	 * @throws IllegalArgumentException if {@code value < 0} or {@code value} is too big
	 * to fit an unsigned 8-byte int (that is, if {@code value >= (1L << 64)}).
	 */
	...
	/**
	 * Return the smallest bytes value whose bytes correspond to the provided long. That
	 * is, the returned value may be of size less than 8 if the provided long has leading
	 * zero bytes.
	 ...
	 *
	 * <p>
	 * This method is lenient in that {@code str} may of an odd length, in which case it
	 ...
	 * @throws IllegalArgumentException if {@code str} does not correspond to a valid
	 ...
	 *
	 * <p>
	 * This method allows for {@code str} to have an odd length, in which case it will
	 * behave exactly as if it had an additional 0 in front.
	 * @param str The hexadecimal string to parse, which may or may not start with "0x".
	 * @param destinationSize The size of the returned value, which must be big enough to
	 * hold the bytes represented by {@code str}. If it is strictly bigger those bytes
	 * from {@code str}, the returned value will be left padded with zeros.
	 * @return A value of size {@code destinationSize} corresponding to {@code str}
	 * potentially left-padded.
	 * @throws IllegalArgumentException if {@code str} does not correspond to a valid
	 * hexadecimal representation, represents more bytes than {@code destinationSize} or
	 ...
	 * @return The value corresponding to {@code str}.
	 * @throws IllegalArgumentException if {@code str} does not correspond to a valid
	 * hexadecimal representation, or is of an odd length.
	 */
	 ...
	 * @return A value of size {@code destinationSize} corresponding to {@code str}
	 * potentially left-padded.
	 * @throws IllegalArgumentException if {@code str} does correspond to a valid
	 * hexadecimal representation, or is of an odd length.
	 * @throws IllegalArgumentException if {@code str} does not correspond to a valid
	 ...
	 * @param i The index from which to get the int, which must less than or equal to
	 * {@code size() - 4}.
	 * @return An integer whose value is the 4 bytes from this value starting at index
	 * {@code i}.
	 ...
	 * @param i The index from which to get the long, which must less than or equal to
	 * {@code size() - 8}.
	 * @return A long whose value is the 8 bytes from this value starting at index
	 * {@code i}.
	 ...
	 * signed integer.
	 * @return A {@link BigInteger} corresponding to interpreting these bytes as a
	 ...
	 * @return A positive (or zero) {@link BigInteger} corresponding to interpreting these
	 ...
	 * or {@code size() * 8} if all bits * are zero.
	 ...
	 *
	 * If this value and the supplied value are different lengths, then the shorter will
	 * be zero-padded to the left.
	 ...
	 *
	 * <p>
	 * If this value or the supplied value are shorter in length than the output vector,
	 * then they will be zero-padded to the left. Likewise, if either this value or the
	 * supplied valid is longer in length than the output vector, then they will be
	 ...
	 *
	 * <p>
	 * If this value is shorter in length than the output vector, then it will be
	 * zero-padded to the left. Likewise, if this value is longer in length than the
	 * output vector, then it will be truncated to the left.
	 * @param result The mutable output vector for the result.
	 * @param <T> The {@link MutableBytes} value type.
	 * @return The {@code result} output vector.
	 */
	 ...
	 * If this value is shorter in length than the output vector, then it will be
	 * zero-padded to the left. Likewise, if this value is longer in length than the
	 * output vector, then it will be truncated to the left (after shifting).
	 * @param distance The number of bits to shift by.
	 * @param result The mutable output vector for the result.
	 * @param <T> The {@link MutableBytes} value type.
	 * @return The {@code result} output vector.
	 ...
	 * more memory than the slide represents. Use {@link #copy} on the returned slice if
	 * that is not what you want.
	 * @param i The start index for the slice.
	 * @return A new value providing a view over the bytes from index {@code i} (included)
	 * to the end.
	 ...
	 * @return A value, equals to this one, but deeply immutable and that doesn't retain
	 * any "unreachable" bytes. For performance reasons, this is allowed to return this
	 * value however if it already fit those constraints.
	 */
	Bytes copy();

	/**
	 * Return a new mutable value initialized with the content of this value.
	 * @return A mutable copy of this value. This will copy bytes, modifying the returned
	 * value will <b>not</b> modify this value.
	 */
	 ...
	 * size.
	 * @param destination The mutable value to which to copy the bytes to, which must have
	 * the same size as this value. If you want to copy value where size differs, you
	 * should use {@link #slice} and/or {@link MutableBytes#mutableSlice} and apply the
	 * copy to the result.
	 * @throws IllegalArgumentException if {@code this.size() != destination.size()}.
	 */
	default void copyTo(MutableBytes destination) {
		checkNotNull(destination);
		checkArgument(destination.size() == size(), "Cannot copy %s bytes to destination of non-equal size %s", size(),
				destination.size());
	 ...
	 * @param destination The mutable value to which to copy the bytes to, which must have
	 * enough bytes from {@code destinationOffset} for the copied value.
	 * @param destinationOffset The offset in {@code destination} at which the copy
	 * starts.
	 * @throws IllegalArgumentException if the destination doesn't have enough room, that
	 * is if {@code
	...
	default void copyTo(MutableBytes destination, int destinationOffset) {
		...
		// Special casing an empty source or the following checks might throw (even though
		// we have
		// nothing to copy anyway) and this gets inconvenient for generic methods using
		// copyTo() as
	 ...
	 * @throws BufferOverflowException If the writer attempts to write more than the
	 * provided buffer can hold.
	 ...
	 *
	 * <p>
	 * Note that since a Vert.x {@link Buffer} will grow as necessary, this method never
	 * fails.
	 ...
	 * @return {@code value} if its left-most byte is non zero, or a slice that exclude
	 ...
	 *
	 * <p>
	 * Contrarily to {@link #toArray()}, this may avoid allocating a new array and
	 ...
	 * "unsafe" moniker.
	 * @return A byte array with the same content than this value, which may or may not be
	 * the direct backing of this value.
	 */
	 ...
	 * @return This value represented as a minimal hexadecimal string (without any leading
	 * zero, except if it's valued zero or empty, in which case it returns 0x0).
	 */
